...
ek2z_s = Gs*pi*(eODs_2^4-eIDs_2^4)/32;          % Torsional stiffness of SS tube 1
...
tors_comp = zeros(length(alpha_L),10);
...
plot(tors_comp(:,2)+tors_comp(:,7)-tors_comp(:,8),tors_comp(:,1),'-','LineWidth',1.5,'Color',[0.9290 0.6940 0.1250]);
hold on

% Optimally designed SS tube - point C
plot(tors_comp(:,2)+tors_comp(:,9)-tors_comp(:,10),tors_comp(:,1),'-b','LineWidth',1.5);
hold on

% Empirically designed SS tube - point A
